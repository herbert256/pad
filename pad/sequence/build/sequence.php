<?php
  ...
  $padSeqTmp = array_key_first($padPrm [$pad]) ?? '';
  if ( $padPrm [$pad] [0] == '' and $padTag [$pad] == 'sequence' and padValid($padSeqTmp) and file_exists ( PAD . "pad/sequence/actions/$padSeqTmp.php" ) )
    return include 'sequence/action.php';
  ...
  if ( strpos($padPrm [$pad] [0], '..') ) {
    $padSeqParm = $padPrm [$pad] [0];
    return include 'sequence/range.php';
  } 

  if ( ctype_digit($padPrm [$pad] [0]) ) {
    $padSeqParm = "1..$padPrm [$pad] [0]";
    return include 'sequence/range.php';
  } 
