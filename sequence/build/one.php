<?php
  ...
  if ( is_null($pad_sequence) or $pad_sequence === FALSE ) return false;
  if ( $pad_seq_to and $pad_sequence > $pad_seq_to       ) return false;

  if ( $pad_seq_unique ) {
    $pad_seq_unique_cnt++;
    if ( $pad_seq_unique_cnt > $pad_seq_unique   ) return false;
    if ( in_array ($pad_sequence, $pad_seq_base) ) return true;
  ...
  if ( $pad_seq_value and ! in_array ($pad_sequence, $pad_seq_value)      ) goto skip;
  if ( $pad_seq_row   and ! in_array (count($pad_seq_base), $pad_seq_row) ) goto skip;
...
go:
  ...
  if ( $pad_seq_row   and count($pad_seq_row)   == count($pad_seq_result) ) return false;
