<?php
  ...
  function padEvalResult ( $result, $value, $eval ) {
    ...
    // A shape that is not one value is reported under the strict syntax check; the
    // lenient answer for what cannot be evaluated is ''.

    if ( count ($result) != 1 or $result[$key][1] != 'VAL' ) {
