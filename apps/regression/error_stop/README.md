# Regression: the 'stop' error action

## Introduction

Regression test for `$padErrorAction = 'stop'`. The boom page raises an engine-level PHP
...
| `_config/config.php` | Chooses the 'stop' action, `_common` off |
