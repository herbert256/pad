<?php
  ...
  function padTypeCheck ( $type, $item ) {
    ...
    if     (                       ! file_exists     ( PAD . "pad/tag/$type.php"              ) ) return FALSE;
    elseif ( $type == 'app'      and file_exists     ( APP . "tags/$type.php"                 ) ) return TRUE;
   ...
   # elseif ( $type == 'include'  and file_exists     ( APP . "includes/$type.html"            ) ) return TRUE;
    ...
    elseif ( $type == 'data'     and isset           ( $GLOBALS ['padDataStore'] [$type]      ) ) return TRUE;
    elseif ( $type == 'sequence' and file_exists     ( PAD . "pad/sequence/types/$type"       ) ) return TRUE;
    elseif ( $type == 'action'   and file_exists     ( PAD . "pad/sequence/actions/$type.php" ) ) return TRUE;
    elseif ( $type == 'store'    and isset           ( $GLOBALS ['padSeqStore'] [$type]       ) ) return TRUE;
    elseif ( $type == 'array'    and padArrayCheck   ( $type                                  ) ) return TRUE;
    elseif ( $type == 'field'    and padFieldCheck   ( $type                                  ) ) return TRUE;
    elseif ( $type == 'parm'     and padParmCheck    ( $type, 1                               ) ) return TRUE;
    elseif ( $type == 'constant' and defined         ( $type                                  ) ) return TRUE;
    elseif ( $type == 'function' and file_exists     ( APP . "functions/$type.php"            ) ) return TRUE;
    elseif ( $type == 'function' and file_exists     ( PAD . "pad/functions/$type.php"        ) ) return TRUE;
    elseif ( $type == 'php'      and function_exists ( $type                                  ) ) return TRUE;
    elseif ( $type == 'object'   and padIsObject     ( $type                                  ) ) return TRUE;
    elseif ( $type == 'resource' and padIsResource   ( $type                                  ) ) return TRUE;
    else                                                                                          return FALSE;
